 ...
 */
public class Temp_JogoInteresseCRUD {

    private SQLiteDatabase db;
    private PersistenceHelper banco;

    public Temp_JogoInteresseCRUD(Context context){
    ...
    public long inserirJogoColecao(Jogo jogo){
        ...
        try {
            ...
            resultado = db.insert("temp_jogointeresse", null, contentValues);
    ...
    public long removerTodaColecao(){
        ...
        db.beginTransaction();
        try{
            resultado = db.delete("temp_jogointeresse", where, null);
            db.setTransactionSuccessful();
        }catch (Exception e){
            db.endTransaction();
            throw  e;
    ...
    public ListaJogos listarJogoInteresse() {
        String sql = "select * from temp_jogointeresse";
